"""
OpenBalance Dashboard (Versão 2.0 - Centro Dinâmico)

Descrição: Dashboard para controlo de bola numa plataforma inclinável,
com deteção por visão computacional e comunicação em tempo real com Arduino.

Autor: João Pavão

Funcionalidades principais:
- Interface gráfica robusta em CustomTkinter com arquitetura modular.
- Comunicação entre threads com total segurança (locks e queues), evitando congelamentos.
- Deteção de bola via HSV ajustável e visualização em tempo real (OpenCV).
- Controlo serial para ativar/desativar motores e seguimento PID.
- Definição dinâmica do centro de referência através de clique do rato.
- Dupla representação gráfica: cruz amarela (centro da imagem) e cruz rosa (centro funcional clicado).
- Botão “Reset Centro” para reposicionar facilmente o centro funcional no meio da imagem.

Versão ideal para testes precisos de controlo e demonstrações didáticas.
"""
...
class VideoHandler:
    ...
    def __init__(self, parent, hsv_frame_ref: HSVSettingsFrame, serial_manager: SerialManager, camera_index=0):
        ...
        self.canvas.bind("<Button-1>", self._on_mouse_click); self.update_hsv_from_frame(); self._initialize_camera()
    ...
    def reset_center(self):
        """Reinicia o centro ao meio da imagem."""
        with self.shared_lock:
            self.center_x = VIDEO_WIDTH // 2
            self.center_y = VIDEO_HEIGHT // 2

    def _on_mouse_click(self, event):
        """Atualiza o centro de referência com base no clique do utilizador."""
        with self.shared_lock:
...
class OpenBalanceApp:
    def __init__(self):
        ...
        self.app = ctk.CTk(); self.app.title("OpenBalance"); self.app.geometry(f"{WINDOW_WIDTH}x{WINDOW_HEIGHT}")
