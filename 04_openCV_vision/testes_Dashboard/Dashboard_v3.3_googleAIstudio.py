"""
OpenBalance Dashboard (Versão 3.3 - Edição Académica com Modo "Sem Filtro")
==========================================================================

Descrição:
    Dashboard para o controlo em tempo real de uma plataforma de equilíbrio de bola.
    Esta versão introduz uma funcionalidade crítica de usabilidade: um modo "Sem Filtro"
    que permite ao utilizador desativar todo o processamento de imagem para visualizar
    o feed de vídeo puro, facilitando a calibração e a depuração.
...
# (Importações e Constantes Globais - Sem alterações em relação à v3.2)
import json; import threading; import csv; import cv2; import numpy as np; import customtkinter as ctk; import serial; import serial.tools.list_ports; from PIL import Image, ImageTk; import tkinter as tk; from tkinter import filedialog, messagebox
WINDOW_WIDTH = 1500; WINDOW_HEIGHT = 850; VIDEO_WIDTH = 800; VIDEO_HEIGHT = 600
DEFAULT_BAUDRATE = 9600; CONFIG_FILE_TYPES = [("JSON Files", "*.json")]; CSV_FILE_TYPES = [("CSV Files", "*.csv")]
CMD_MOTORS_ON = "M1\n"; CMD_MOTORS_OFF = "M0\n"; CMD_ERROR_PREFIX = "E"; CMD_PID_PREFIX = "PID"
...
# (SerialManager e StatusBar - Sem alterações)
class SerialManager:
    ...
    def connect(self, port, baudrate=DEFAULT_BAUDRATE):
        try:
            ...
            return True
        except Exception as e: self.serial_conn = None; print(f"Erro ao conectar na porta serial: {e}"); return False
    def disconnect(self):
        if self.serial_conn and self.serial_conn.is_open: self.serial_conn.close()
...
# -----------------------------------------------------------------------------
# Classe HSVSettingsFrame (ALTERADA)
# -----------------------------------------------------------------------------
class HSVSettingsFrame(ctk.CTkFrame):
    def __init__(self, parent, video_handler, *args, **kwargs):
        ...
        ## ALTERAÇÃO DIDÁTICA v3.3: Gestão de Estado do Filtro ##
        # Esta variável de estado é a chave da nova funcionalidade.
        # Quando True, o processamento ocorre. Quando False, é ignorado.
        ...
        preset_frame = ctk.CTkFrame(self, fg_color="transparent"); preset_frame.grid(row=14, column=0, sticky="ew", padx=10); preset_frame.grid_columnconfigure((0, 1, 2), weight=1)
        
        ## ALTERAÇÃO DIDÁTICA v3.3: Adição do botão "Sem Filtro" ##
        # Criamos o novo botão e associamo-lo a uma função que desativa o filtro.
        ...
        # Os outros botões são criados dinamicamente.
        color_items = list(BUTTON_COLORS.items())
        for i in range(len(color_items)):
            color_name, colors = color_items[i]
            # Ajustamos o cálculo da posição para acomodar o novo botão
    ...
    ## ALTERAÇÃO DIDÁTICA v3.3: Método para controlar o estado dos widgets ##
    def _toggle_filter_controls(self, enabled):
        """Ativa ou desativa os controlos de HSV com base no estado do filtro."""
        state = "normal" if enabled else "disabled"
        for slider in self.hsv_sliders:
            slider.configure(state=state)
        self.checkbox_mask.configure(state=state)
        # Atualiza a nossa variável de estado.
        self.filter_enabled = enabled

    def _disable_filter(self):
        """Callback do botão "Sem Filtro". Desativa os controlos."""
        self._toggle_filter_controls(False)
        if self.video_handler: self.video_handler.update_once()

    def _apply_preset(self, color_name):
        """Callback dos presets de cor. Ativa os controlos antes de aplicar os valores."""
        ## ALTERAÇÃO DIDÁTICA v3.3: Reativa os controlos ##
        # Garante que, ao selecionar uma cor, o modo "Sem Filtro" é desativado.
    ...
    # (Resto da classe HSVSettingsFrame sem alterações)
...
class VideoHandler:
    ...
    def _process_and_draw(self, frame_bgr):
        ## ALTERAÇÃO DIDÁTICA v3.3: Lógica condicional de processamento ##
        # Este `if` é o ponto de controlo central. Se o filtro estiver desativado,
        # ele executa um subconjunto de operações muito simples e retorna,
        # ignorando todo o pipeline de visão computacional.
        if not self.hsv_frame.filter_enabled:
            # No modo "Sem Filtro", apenas mostramos a imagem original com a cruz.
            self.servo_x_var.set("Servo X: (sem filtro)")
            self.servo_y_var.set("Servo Y: (sem filtro)")
            display_img = cv2.cvtColor(frame_bgr, cv2.COLOR_BGR2RGB)
        else:
            # Se o filtro estiver ATIVO, executa o pipeline completo como antes.
            ...
            if contours:
                ...
                if cv2.contourArea(c) > 500:
                    ...
                    temp_display_img = cv2.cvtColor(frame_bgr, cv2.COLOR_BGR2RGB) if not self.show_mask else cv2.cvtColor(mask, cv2.COLOR_GRAY2RGB)
                    cv2.circle(temp_display_img, (int(x), int(y)), int(radius), (0, 255, 0), 2); cv2.circle(temp_display_img, (int(x), int(y)), 3, (255, 0, 0), -1)
                    display_img = temp_display_img
        ...
        # A cruz de referência é desenhada em AMBOS os modos.
        ...
        # A exibição da imagem no canvas é comum a ambos os modos.
    ...
    # (Resto da classe VideoHandler sem alterações)
...
# (As classes PIDControllerFrame e OpenBalanceApp não necessitam de alterações nesta versão,
# pois a lógica está contida em HSVSettingsFrame e VideoHandler. As incluímos para
# que o código esteja completo.)
...
class PIDControllerFrame(ctk.CTkFrame):
    ...
    def _atualizar_portas(self): novas_portas = self.serial_manager.list_ports(); self.option_menu_porta.configure(values=novas_portas if novas_portas else ["Nenhuma porta"]); self.porta_var.set(novas_portas[0] if novas_portas else "Nenhuma porta"); self.app.status_bar.show_message("Lista de portas seriais atualizada.")
...
class OpenBalanceApp:
    def __init__(self):
        ...
        self.app = ctk.CTk(); self.app.title("OpenBalance Dashboard 3.3"); self.app.geometry(f"{WINDOW_WIDTH}x{WINDOW_HEIGHT}")
        self.app.grid_rowconfigure(0, weight=1); self.app.grid_rowconfigure(1, weight=0)
        self.app.grid_columnconfigure(0, weight=1, minsize=320); self.app.grid_columnconfigure(1, weight=4, minsize=VIDEO_WIDTH + 40); self.app.grid_columnconfigure(2, weight=1, minsize=320)
    ...
    def _create_frames(self):
        ...
        frame_video_container.grid_columnconfigure(0, weight=1); frame_video_container.grid_rowconfigure(2, weight=1)
        ...
        self.pid_frame = PIDControllerFrame(self.app, self.serial_manager, self, corner_radius=10); self.pid_frame.grid(row=0, column=2, sticky="nswe", padx=10, pady=10)
