...
"""
================================================================================
OpenBalance Dashboard (Versão 5.0 - Edição de Controlo de Precisão)
================================================================================

Descrição:
    Dashboard de controlo para uma plataforma de equilíbrio de bola. A Versão 5.0
    representa um salto qualitativo significativo, introduzindo um Filtro de
    Kalman para o processamento do sinal de posição da bola. Esta técnica
    avançada de filtragem suaviza o ruído (jitter) inerente à deteção por
    visão computacional, resultando num movimento dos servos drasticamente mais
    suave e num controlo geral mais estável e preciso.

Arquitetura e Melhorias Notáveis (v5.0):
    - **Implementação do Filtro de Kalman:** Uma nova classe `KalmanFilter` foi
      adicionada, utilizando a implementação robusta do OpenCV para estimar com
      precisão o estado da bola (posição e velocidade).
    - **Sinal de Controlo Suavizado:** O `VideoHandler` agora filtra a posição
      "crua" da bola através do filtro de Kalman antes de a enviar para os
      controladores PID.
    - **Controlo de Filtro na UI:** Uma checkbox foi adicionada à interface,
      permitindo ativar ou desativar o Filtro de Kalman em tempo real para
      comparar o seu efeito.
    - **Feedback Visual Aprimorado:** O vídeo agora desenha dois círculos sobre a
      bola: um para a posição medida (crua) e outro para a posição estimada
      (filtrada), tornando o efeito do filtro imediatamente visível.
    - **Mantém Todas as Funcionalidades Anteriores:** PID por eixo independente e
      o assistente de calibração por graus continuam totalmente funcionais.
...
# -----------------------------------------------------------------------------
# 1. IMPORTAÇÕES E CONSTANTES GLOBAIS (Sem alterações)
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# 2. CLASSES UTILITÁRIAS (Com adição da KalmanFilter)
# -----------------------------------------------------------------------------
class SerialManager: # Sem alterações
...
class StatusBar(ctk.CTkFrame): # Sem alterações
...
class PIDController: # Sem alterações
...
class KalmanFilter:
    ...
    """
    def __init__(self):
        # dynamParams=4 (estado: pos, vel), measureParams=2 (medição: pos, vel)
        self.kalman = cv2.KalmanFilter(4, 2)
        
        # Matriz de Medição (H): Mapeia o estado para a medição. Medimos pos e vel.
        self.kalman.measurementMatrix = np.array([[1, 0, 0, 0], [0, 1, 0, 0]], np.float32)

        # Matriz de Transição (A): Modela como o estado evolui no tempo.
        # Assumimos um modelo de velocidade constante para simplicidade.
    ...
    def predict(self):
        """Prevê a próxima posição e velocidade do objeto."""
        prediction = self.kalman.predict()
        return (prediction[0, 0], prediction[1, 0]) # pos, vel

    def correct(self, measurement):
        """Corrige a previsão com base na medição atual (posição e velocidade)."""
        measurement_vec = np.array([[np.float32(measurement[0])], [np.float32(measurement[1])]])
        corrected = self.kalman.correct(measurement_vec)
        return (corrected[0, 0], corrected[1, 0]) # pos, vel

# -----------------------------------------------------------------------------
# 3. CLASSES DE INTERFACE GRÁFICA (FRAMES)
# -----------------------------------------------------------------------------
class HSVSettingsFrame(ctk.CTkFrame): # Sem alterações
...
class VideoHandler:
    def __init__(self, parent, hsv_frame, serial_manager, app_ref):
        ...
        self.kalman_x = KalmanFilter(); self.kalman_y = KalmanFilter()
    ...
    def _process_and_draw(self, frame_bgr):
        ...
        self.kalman_x.predict(); self.kalman_y.predict()
        ...
        if self.hsv_frame.filter_enabled:
            ...
            if contours:
                ...
                if cv2.contourArea(c) > 500:
                    ...
                    vel_x = x - self.last_pos_x; vel_y = y - self.last_pos_y
                    corrected_x, _ = self.kalman_x.correct((x, vel_x)); corrected_y, _ = self.kalman_y.correct((y, vel_y))
                    self.last_pos_x = x; self.last_pos_y = y
                    
                    final_x, final_y = (corrected_x, corrected_y) if self.kalman_enabled.get() else (x, y)
                    
                    erro_x = int(final_x) - (VIDEO_WIDTH // 2); erro_y = -(int(final_y) - (VIDEO_HEIGHT // 2))
                    self.servo_x_var.set(f"Erro X: {erro_x}"); self.servo_y_var.set(f"Erro Y: {erro_y}")
            ...
            if not ball_detected:
                self.servo_x_var.set("Erro X: (perdido)"); self.servo_y_var.set("Erro Y: (perdido)")
                self.pid_x_var.set("PID X: 0.00"); self.pid_y_var.set("PID Y: 0.00")
        else:
            self.servo_x_var.set("Erro X: (sem filtro)"); self.servo_y_var.set("Erro Y: (sem filtro)")
...
class ControlFrame(ctk.CTkFrame): # Sem alterações na lógica, apenas na inicialização
    ...
    def _create_operation_tab(self, tab):
        ...
        pid_group_x, widgets_x = self._create_pid_group(tab, "X"); pid_group_x.grid(row=0, column=0, padx=10, pady=5, sticky="ew"); self.pid_params_widgets["X"] = widgets_x
        pid_group_y, widgets_y = self._create_pid_group(tab, "Y"); pid_group_y.grid(row=1, column=0, padx=10, pady=5, sticky="ew"); self.pid_params_widgets["Y"] = widgets_y
    ...
    def _calculate_and_apply_calibration(self):
        ...
        except (ValueError, ZeroDivisionError) as e: self.app.status_bar.show_message(f"Erro no cálculo: Verifique se todos os valores são números válidos. ({e})", is_error=True)
...
class OpenBalanceApp:
    def __init__(self):
        ...
        self.app = ctk.CTk(); self.app.title("OpenBalance Dashboard 5.0 - Edição de Controlo de Precisão"); self.app.geometry(f"{WINDOW_WIDTH}x{WINDOW_HEIGHT}")
    ...
    def _load_app_config(self):
        ...
        try:
            ...
            self.status_bar.show_message("config.json não encontrado.", is_error=True)
