"""
OpenBalance Dashboard (Versão com Botões Coloridos para Presets)
...
envio de coordenadas detectadas via serial para controle dos servos, calibração básica,
conexão serial e exportação de CSV.
...
HSV_PRESETS = {
    ...
    "Cinzento": {
        # Para bola metálica (cinzenta), baixa saturação e valor intermediário
        "h_min": 0,   "h_max": 180,
        "s_min": 0,   "s_max": 50,
        "v_min": 50,  "v_max": 200
    },
    "Azul": {
        # Matiz aproximado para azul, saturação alta, valor médio-alto
        "h_min": 90,  "h_max": 130,
        "s_min": 100, "s_max": 255,
        "v_min": 50,  "v_max": 255
    },
    "Branco": {
        # Baixa saturação, valor muito alto
        "h_min": 0,   "h_max": 180,
        "s_min": 0,   "s_max": 30,
        "v_min": 200, "v_max": 255
    }
...
class SerialManager:
    ...
    def send(self, data_str):
        ...
        :param data_str: string formatada com coordenadas, ex: "B,120,240\n"
        """
...
class HSVSettingsFrame(ctk.CTkFrame):
    ...
    def __init__(self, parent, video_handler, *args, **kwargs):
        ...
        # Botões de presets de cor (com cores correspondentes)
        row_base = 14
    ...
    def _apply_preset(self, color_name):
        ...
        :param color_name: chave em HSV_PRESETS
        """
...
class PIDControllerFrame(ctk.CTkFrame):
    ...
    def __init__(self, parent, serial_manager: SerialManager, *args, **kwargs):
        ...
        ctk.CTkLabel(self, text="", height=10).grid(row=21, column=0)
        ctk.CTkButton(
            self,
...
class VideoHandler:
    ...
    de captura baseado em OpenCV. Alterna entre vídeo normal e máscara HSV, desenha
    um círculo ao redor da bola detectada e envia coordenadas do centro via serial.
    """
    def __init__(self, parent, hsv_frame: HSVSettingsFrame, serial_manager: SerialManager):
        self.parent = parent
    ...
    def _process_and_draw(self, frame_bgr):
        ...
        if contours:
            ...
            if area > 500:
                ...
                if self.serial_manager and self.serial_manager.serial_conn and self.serial_manager.serial_conn.is_open:
                    cmd_str = f"B,{ball_center[0]},{ball_center[1]}\n"
        ...
        if ball_center and ball_radius:
            ...
            self.servo_x_var.set(f"Servo X: {ball_center[0]:.0f}°")
            self.servo_y_var.set(f"Servo Y: {ball_center[1]:.0f}°")
...
class OpenBalanceApp:
    ...
    def _create_frames(self):
        ...
        self.video_handler = VideoHandler(frame_video_container, self.hsv_frame, self.serial_manager)
    ...
    def _mostrar_sobre(self):
        messagebox.showinfo(
            "Sobre",
            "OpenBalance Dashboard\nAutor: João Pavão\nVersão: 1.0\nRefatorado com botões coloridos para presets de cor"
        )
