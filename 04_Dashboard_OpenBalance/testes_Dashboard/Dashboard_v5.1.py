...
"""
================================================================================
OpenBalance Dashboard (Versão 5.2 - Edição de Controlo Refinado)
================================================================================

Descrição:
    Dashboard de controlo para uma plataforma de equilíbrio de bola. A Versão 5.2
    introduz correções críticas de controlo e melhorias de usabilidade,
    solidificando a base estável da v5.0.

Arquitetura e Melhorias Notáveis (v5.2):
    - **CORREÇÃO DA LÓGICA PID:** A lógica de cálculo do erro foi ajustada para
      garantir um feedback negativo adequado. Anteriormente, o sistema exibia
      feedback positivo (a plataforma seguia a bola). Agora, a plataforma move-se
      corretamente para contrariar o movimento da bola, estabilizando-a no centro.
    - **BOTÃO DE NIVELAMENTO:** Foi adicionado um novo botão "Nivelar Plataforma"
      ao separador "Operação". Este comando move os servos para a posição de
      equilíbrio definida na calibração, facilitando a preparação para um teste.
    - **CÓDIGO EXTENSIVAMENTE COMENTADO:** A pedido, o código foi revisto para
      adicionar comentários detalhados e melhorar a formatação, tornando-o
      mais legível e fácil de manter.
    - **Mantém Todas as Funcionalidades Anteriores:** Filtro de Kalman, PID por
      eixo independente e o assistente de calibração continuam totalmente
      funcionais e aprimorados pela nova estabilidade do controlo.
...
class PIDController:
    ...
    def update(self, process_variable):
        ...
        Espera receber a 'process_variable' (ex: posição da bola) e não um erro pré-calculado.
        """
    ...
    def set_gains(self, Kp, Ki, Kd):
        self.Kp, self.Ki, self.Kd = Kp, Ki, Kd
        self._integral = 0  # Resetar o termo integral ao mudar os ganhos
...
class VideoHandler:
    ...
    def __init__(self, parent, hsv_frame, serial_manager, app_ref):
        ...
        # Um único filtro de Kalman para rastrear o estado 2D (posição x, y)
    ...
    def _process_and_draw(self, frame_bgr):
        ...
        if self.hsv_frame.filter_enabled:
            ...
            # Operações morfológicas para limpar o ruído da máscara
            ...
            if contours:
                ...
                if cv2.contourArea(c) > 500: # Filtrar contornos muito pequenos
                    ball_detected = True
                    ((x, y), radius) = cv2.minEnclosingCircle(c)
                    
                    # Corrigir a estimativa do Kalman com a nova medição
                    corrected_pos = self.kalman_filter.correct((x, y))
                    corrected_x, corrected_y = corrected_pos[0], corrected_pos[1]
                    
                    # Escolher entre posição filtrada ou crua com base na checkbox
                    final_x, final_y = (corrected_x, corrected_y) if self.kalman_enabled.get() else (x, y)
                    
                    # --- CORREÇÃO CRÍTICA v5.2 ---
                    # O cálculo do erro foi invertido para (Setpoint - Posição_Atual).
                    # A classe PID calcula internamente 'setpoint(0) - process_variable'.
                    # Ao passar um erro já calculado, ocorria uma dupla inversão,
                    # resultando em feedback positivo (a plataforma seguia a bola).
                    # Esta correção garante o feedback negativo correto.
                    erro_x = (VIDEO_WIDTH // 2) - int(final_x)
                    erro_y = (VIDEO_HEIGHT // 2) - int(final_y)
                    ...
                    # Desenhar a posição medida (crua) em vermelho
                    cv2.circle(display_img, (int(x), int(y)), int(radius), (255, 0, 0, 128), 2)
                    # Desenhar a posição estimada (filtrada) em verde
    ...
    def send_servo_command(self, pid_output_x, pid_output_y):
        """Envia o comando final aos servos, após mapear a saída do PID."""
        calib = self.app.calibration_data
        pid_min, pid_max = -1000, 1000  # Gama de saída esperada do PID

        # --- CORREÇÃO CRÍTICA v5.2.1 ---
        # A montagem mecânica pode fazer com que um aumento no sinal do servo (μs)
        # incline a plataforma na direção errada (feedback positivo).
        # Para corrigir isto, invertemos a gama de mapeamento do servo.
        # Em vez de mapear para [min, max], mapeamos para [max, min].
        # Isto inverte a resposta do controlo para corresponder à realidade física.

        # Mapeia a saída do PID para a gama de microssegundos do servo, de forma INVERTIDA.
        target_x_us = self.map_value(
            pid_output_x,
            pid_min, pid_max,
            calib['x_max_us'], calib['x_min_us']  # Gama invertida: max -> min
        ) + calib['x_center_offset_us']

        target_y_us = self.map_value(
            pid_output_y,
            pid_min, pid_max,
            calib['y_max_us'], calib['y_min_us']  # Gama invertida: max -> min
    ...
    def map_value(v, f_min, f_max, t_min, t_max):
        """Mapeia um valor de uma gama para outra (como o map() do Arduino)."""
        return (v - f_min) * (t_max - t_min) / (f_max - f_min) + t_min
        
    def _initialize_camera(self):
        self.stop() # Garante que a thread anterior foi terminada
        ...
        while self.running:
            if self.cap and self.cap.isOpened():
                ...
                if ret:
                    ...
                    # Se a leitura falhar, para o loop para evitar erros
                    self.running = False
            else:
                self.running = False
    ...
    def update_once(self):
        """Força uma única atualização do frame, útil quando o vídeo está pausado."""
        if self.cap and self.cap.isOpened():
            ret, frame = self.cap.read()
            if ret:
                self._process_and_draw(frame)
...
class ControlFrame(ctk.CTkFrame):
    ...
    def _create_operation_tab(self, tab):
        ...
        # --- ADIÇÃO v5.2 ---
        # Botão para nivelar a plataforma de acordo com a calibração
        ...
        for i, param in enumerate(["Kp", "Ki", "Kd"]):
            ...
            def update_from_entry(e, p=param, ax=axis_name):
                try:
                    ...
                    pass # Ignora erros de digitação
    ...
    # --- Métodos de Lógica e Controlo ---
    
    def _center_platform(self):
        """(NOVO v5.2) Envia um comando para mover os servos para a posição de equilíbrio definida na calibração."""
...
class OpenBalanceApp:
    ...
    def __init__(self):
        ...
        self.app = ctk.CTk()
        self.app.title("OpenBalance Dashboard 5.2 - Edição de Controlo Refinado")
    ...
    def _create_frames(self):
        # Frame de configurações HSV (esquerda)
        ...
        # Frame para seleção de câmara e controlo do filtro
        ...
        # Inicializa o VideoHandler, que cria o seu próprio canvas
        self.video_handler = VideoHandler(frame_video_container, self.hsv_frame, self.serial_manager, self)
        self.hsv_frame.video_handler = self.video_handler # Link de volta para o hsv_frame
        
        # Checkbox para o Filtro de Kalman
        ...
        # Carregar configurações salvas nos frames
